...
impl GmlReader {
    ...
    pub fn set_source_crs(&mut self, crs: &str) {
        self.source_crs = Some(crs.to_string());
        self.update_transformer();
    }

    pub fn set_target_crs(&mut self, crs: &str) {
        self.target_crs = Some(crs.to_string());
        self.update_transformer();
    }

    fn update_transformer(&mut self) {
        if let (Some(source), Some(target)) = (&self.source_crs, &self.target_crs) {
            self.transformer = Proj::new_known_crs(
                source,
                target,
                None
            ).ok();
    ...
    fn parse_coordinates(&self, coords_text: &str) -> Result<Option<Geometry<f64>>, Box<dyn Error>> {
        ...
        for coord_pair in coords_text.split_whitespace() {
            ...
            if coords.len() >= 2 {
                let mut x = coords[0].parse::<f64>()?;
                let mut y = coords[1].parse::<f64>()?;

                if let Some(proj) = &self.transformer {
                    let (transformed_x, transformed_y) = proj.convert((x, y))?;
                    x = transformed_x;
                    y = transformed_y;
                }
